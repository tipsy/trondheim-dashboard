...
class EnergyWidget extends BaseWidget {
    ...
    calculateNextHours(nextFourRaw) {
        return nextFourRaw.map((p, idx) => {
            ...
            let prevVal = null;
            if (idx === 0) {
                if (this.currentPrice) prevVal = Math.round(this.currentPrice.NOK_per_kWh * 100);
            } else {
    ...
    renderContent() {
        if (!this.currentPrice && (!this.nextHours || this.nextHours.length === 0)) {
            return html`<p class="no-data">No energy price data available</p>`;
        }

        return html`
            ${this.currentPrice ? html`
