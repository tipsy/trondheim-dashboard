...
export class GeocodingAPI extends APIBase {
  ...
  static async geocodeAddress(address, limit = 20) {
    try {
      ...
      if (
        !lowerAddress.includes("trøndelag") &&
      ...
      const data = await this.fetchJSON("geocoding", {
      ...
      if (!data || data.length === 0) {
        throw new Error(
          'No results found. Try a different address or use "Use Location" button.',
        );
      }
      ...
      if (filteredResults.length === 0) {
        throw new Error(
          "No specific addresses found in Trøndelag. Please include a street number.",
        );
      }
      ...
      return filteredResults.slice(0, 5).map((result) => ({
        ...
        displayName: this.formatNorwegianAddress(
          result.display_name,
          result.address,
        ),
        type: result.type,
      ...
      if (
        ...
        error.message.includes("No specific addresses")
      ) {
        throw error;
      }
      ...
      throw this.handleError(
        error,
        "Could not find address. Please try a different search term.",
      );
    }
  }
  ...
  static async reverseGeocode(lat, lon) {
    try {
      const data = await this.fetchJSON("reverse-geocoding", {
        ...
        (error) => {
          ...
          switch (error.code) {
            ...
            case error.POSITION_UNAVAILABLE:
              errorMessage =
                "Location unavailable - check that Location Services are enabled in System Preferences/Settings";
              break;
            case error.TIMEOUT:
