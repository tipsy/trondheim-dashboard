...
async function loadTranslations(locale) {
  try {
    ...
    conditionalLog(`[Localization] Fetch response:`, response.status, response.ok);
    if (!response.ok) {
...
export async function initLocale() {
  ...
  if (savedLocale && allLocales.includes(savedLocale)) {
    ...
    conditionalLog('[Localization] Loaded', Object.keys(translations).length, 'translations for', currentLocale);
...
export function t(key) {
  conditionalLog(`[t] key="${key}", locale="${currentLocale}", has translation=${!!translations[key]}`);
